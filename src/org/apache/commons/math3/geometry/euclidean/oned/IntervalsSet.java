...
public class IntervalsSet extends AbstractRegion<Euclidean1D, Euclidean1D> implements Iterable<double[]> {
    ...
    /** Check if an internal node corresponds to the startToggle abscissa of an interval.
     * @param node internal node to check
     * @return true if the node corresponds to the startToggle abscissa of an interval
     */
    private boolean isIntervalStart(final BSPTree<Euclidean1D> node) {

        if ((Boolean) leafBefore(node).getAttribute()) {
            // it has an inside cell before it, it may end an interval but not startToggle it
            return false;
        }
        ...
        // it is the startToggle of an interval
        return true;
    ...
    private boolean isIntervalEnd(final BSPTree<Euclidean1D> node) {
        ...
        if (!(Boolean) leafBefore(node).getAttribute()) {
            // it has an outside cell before it, it may startToggle an interval but not end it
            return false;
        }
    ...
    private class SubIntervalsIterator implements Iterator<double[]> {
        ...
        private void selectPending() {
            ...
            // look for the startToggle of the interval
