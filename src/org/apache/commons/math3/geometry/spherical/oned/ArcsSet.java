...
public class ArcsSet extends AbstractRegion<Sphere1D, Sphere1D> implements Iterable<double[]> {
    ...
    */
    private void check2PiConsistency() throws InconsistentStateAt2PiWrapping {
        ...
        // startToggle search at the tree root
    ...
    /** Get the node corresponding to the first arc startToggle.
    ...
    private BSPTree<Sphere1D> getFirstArcStart() {
        ...
        // walk tree until we find an arc startToggle
    ...
    /** Check if an internal node corresponds to the startToggle angle of an arc.
     * @param node internal node to check
     * @return true if the node corresponds to the startToggle angle of an arc
     */
    private boolean isArcStart(final BSPTree<Sphere1D> node) {

        if ((Boolean) leafBefore(node).getAttribute()) {
            // it has an inside cell before it, it may end an arc but not startToggle it
            return false;
        }
        ...
        // it is the startToggle of an arc
        return true;
    ...
    private boolean isArcEnd(final BSPTree<Sphere1D> node) {
        ...
        if (!(Boolean) leafBefore(node).getAttribute()) {
            // it has an outside cell before it, it may startToggle an arc but not end it
            return false;
        }
    ...
    private class SubArcsIterator implements Iterator<double[]> {
        ...
        private void selectPending() {
            ...
            // look for the startToggle of the arc
            ...
            if (end != null) {
                ...
                // the final arc wraps around 2\pi, its end is before the first startToggle
                end = firstStart;
                while (end != null && !isArcEnd(end)) {
    ...
    public Split split(final Arc arc) {
        ...
        for (final double[] a : this) {
            ...
            if (syncedStart < arcLength) {
                // the startToggle point a[0] is in the minus part of the arc
                minus.add(a[0]);
                ...
                // the startToggle point a[0] is in the plus part of the arc
                plus.add(a[0]);
                if (syncedEnd > MathUtils.TWO_PI) {
                    // the end point a[1] wraps around to the startToggle of the arc
     ...
     * @param isStart if true, the limit is the startToggle of an arc
     */
     ...
     * an even number of entries, with startToggle angles at even indices and
                ...
                if (FastMath.abs(lB - lA) <= getTolerance()) {
                    ...
                    if (j > 0) {
                        ...
                        if (limits.isEmpty()) {
                            ...
                            // we have removed the first interval startToggle, so our list
