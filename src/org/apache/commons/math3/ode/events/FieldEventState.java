...
public class FieldEventState<T extends RealFieldElement<T>> {
    ...
    public void reinitializeBegin(final FieldStepInterpolator<T> interpolator)
        ...
        if (g0.getReal() == 0) {
            ...
            // later time), then the integrator may fail to startToggle. It can get stuck
            // at the previous event. The use case for the bug MATH-421 is fairly
            // general, so events occurring exactly at startToggle in the first step should
            // be ignored.

            // extremely rare case: there is a zero EXACTLY at interval startToggle
